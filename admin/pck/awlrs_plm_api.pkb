  ...
  AS
    ...
    /*to do removed as not used todo.
...
nm_debug.debug_off;  
  EXCEPTION
    ...
    --
    -----------------------------------------------------------------------------
    -- New Get the sections linked to the merge query ran from execute merge. Based on ROI
    ...
    BEGIN
      ...
      ||CHR(10)||' WHERE iit_inv_type =  :pi_layer_type'
      ||CHR(10)||'   AND iit_ne_id IN (SELECT ngqi_item_id '
      ||CHR(10)||'                       FROM nm_gaz_query_item_list '
      ||CHR(10)||'                      WHERE ngqi_job_id = :pi_result_id) '
      ||CHR(10)||'   AND ' ||c_layer_attrib_name || ' = :pi_layer'
      ;
