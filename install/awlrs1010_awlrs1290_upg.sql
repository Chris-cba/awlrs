...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //new_vm_latest/archives/awlrs/install/awlrs1010_awlrs1290_upg.sql-arc   1.1   Oct 11 2019 16:09:00   Peter.Bibby  $
--       Module Name      : $Workfile:   awlrs1010_awlrs1290_upg.sql  $
--       Date into PVCS   : $Date:   Oct 11 2019 16:09:00  $
--       Date fetched Out : $Modtime:   Oct 11 2019 15:31:14  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='awlrs1010_awlrs1290_1_&log_extension'
define logfile2='awlrs1010_awlrs1290_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'AWLRS'
                          ,p_new_version           => '1.2.9.1'
                          ,p_allowed_old_version_1 => '1.0.1.0'
                          ,p_allowed_old_version_2 => '1.1.8.4'
                          ,p_allowed_old_version_3 => '1.2.8.1'
                          );
END;
/
...
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs1290_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs1290_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('AWLRS','awlrs1010_awlrs1290_upg.sql','Upgrade from 1.0.1.0 to 1.2.9.1','1.2.9.1');
END;
/
COMMIT;
SET HEADING OFF
