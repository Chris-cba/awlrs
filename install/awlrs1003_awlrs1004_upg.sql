...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //new_vm_latest/archives/awlrs/install/awlrs1003_awlrs1004_upg.sql-arc   1.0   04 May 2017 13:56:26   Mike.Huitson  $
--       Module Name      : $Workfile:   awlrs1003_awlrs1004_upg.sql  $
--       Date into PVCS   : $Date:   04 May 2017 13:56:26  $
--       Date fetched Out : $Modtime:   04 May 2017 13:48:12  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='awlrs1003_awlrs1004_1_&log_extension'
define logfile2='awlrs1003_awlrs1004_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'AWLRS'
                          ,p_new_version           => '1.0.0.4'
                          ,p_allowed_old_version_1 => '1.0.0.3'
                          );
END;
/
...
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1003_awlrs1004_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1003_awlrs1004_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('AWLRS','awlrs1003_awlrs1004_upg.sql','Upgrade from 1.0.0.3 to 1.0.0.4','1.0.0.4');
END;
/
COMMIT;
SET HEADING OFF
