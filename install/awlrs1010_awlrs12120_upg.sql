...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //new_vm_latest/archives/awlrs/install/awlrs1010_awlrs12120_upg.sql-arc   1.0   Mar 18 2020 15:54:58   Peter.Bibby  $
--       Module Name      : $Workfile:   awlrs1010_awlrs12120_upg.sql  $
--       Date into PVCS   : $Date:   Mar 18 2020 15:54:58  $
--       Date fetched Out : $Modtime:   Mar 18 2020 15:54:20  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='awlrs1010_awlrs12120_1_&log_extension'
define logfile2='awlrs1010_awlrs12120_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'AWLRS'
                          ,p_new_version           => '1.2.12.0'
                          ,p_allowed_old_version_1 => '1.0.1.0'
                          ,p_allowed_old_version_2 => '1.2.11.0'
                          );
END;
/
...
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs12120_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs12120_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('AWLRS','awlrs1010_awlrs12120_upg.sql','Upgrade from 1.0.1.0 to 1.2.12.1','1.2.12.1');
END;
/
COMMIT;
SET HEADING OFF
