...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //new_vm_latest/archives/awlrs/install/awlrs1010_awlrs13140_upg.sql-arc   1.1   Jul 06 2020 08:49:12   Peter.Bibby  $
--       Module Name      : $Workfile:   awlrs1010_awlrs13140_upg.sql  $
--       Date into PVCS   : $Date:   Jul 06 2020 08:49:12  $
--       Date fetched Out : $Modtime:   Jul 06 2020 08:46:14  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='awlrs1010_awlrs13140_1_&log_extension'
define logfile2='awlrs1010_awlrs13140_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'AWLRS'
                          ,p_new_version           => '1.3.14.0'
                          ,p_allowed_old_version_1 => '1.0.1.0'
                          ,p_allowed_old_version_1 => '1.3.13.1'
                          );
END;
/
...
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs13140_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs13140_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('AWLRS','awlrs1010_awlrs13140_upg.sql','Upgrade to 1.3.14.1','1.3.14.1');
END;
/
COMMIT;
SET HEADING OFF
