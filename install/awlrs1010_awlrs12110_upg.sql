...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //new_vm_latest/archives/awlrs/install/awlrs1010_awlrs12110_upg.sql-arc   1.2   Feb 12 2020 15:20:50   Peter.Bibby  $
--       Module Name      : $Workfile:   awlrs1010_awlrs12110_upg.sql  $
--       Date into PVCS   : $Date:   Feb 12 2020 15:20:50  $
--       Date fetched Out : $Modtime:   Feb 12 2020 15:20:28  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='awlrs1010_awlrs12110_1_&log_extension'
define logfile2='awlrs1010_awlrs12110_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'AWLRS'
                          ,p_new_version           => '1.2.11.0'
                          ,p_allowed_old_version_1 => '1.0.1.0'
                          ,p_allowed_old_version_2 => '1.2.10.1'
                          );
END;
/
...
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs12110_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs12110_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
