...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //new_vm_latest/archives/awlrs/install/awlrs1010_awlrs1140_upg.sql-arc   1.4   Nov 12 2018 10:56:18   Barbara.Odriscoll  $
--       Date into PVCS   : $Date:   Nov 12 2018 10:56:18  $
--       Module Name      : $Workfile:   awlrs1010_awlrs1140_upg.sql  $
--       Date fetched Out : $Modtime:   Nov 12 2018 10:55:32  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='awlrs1010_awlrs1140_1_&log_extension'
define logfile2='awlrs1010_awlrs1140_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'AWLRS'
                          ...
                          ,p_allowed_old_version_3 => '1.1.4.1'
                          );
END;
/
...
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs1140_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs1140_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
