...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //new_vm_latest/archives/awlrs/install/awlrs1010_awlrs13130_upg.sql-arc   1.0   Jun 09 2020 12:08:10   Peter.Bibby  $
--       Module Name      : $Workfile:   awlrs1010_awlrs13130_upg.sql  $
--       Date into PVCS   : $Date:   Jun 09 2020 12:08:10  $
--       Date fetched Out : $Modtime:   Jun 09 2020 12:02:06  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='awlrs1010_awlrs13130_1_&log_extension'
define logfile2='awlrs1010_awlrs13130_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'AWLRS'
                          ,p_new_version           => '1.3.13.0'
                          ,p_allowed_old_version_1 => '1.0.1.0'
                          ,p_allowed_old_version_2 => '1.1.10.0'
                          ,p_allowed_old_version_3 => '1.2.10.0'
                          );
END;
/
...
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs13130_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs13130_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('AWLRS','awlrs1010_awlrs13130_upg.sql','Upgrade to 1.3.13.1','1.3.13.1');
END;
/
COMMIT;
SET HEADING OFF
