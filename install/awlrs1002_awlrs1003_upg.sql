...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //new_vm_latest/archives/awlrs/install/awlrs1002_awlrs1003_upg.sql-arc   1.0   08 Mar 2017 16:18:54   Mike.Huitson  $
--       Module Name      : $Workfile:   awlrs1002_awlrs1003_upg.sql  $
--       Date into PVCS   : $Date:   08 Mar 2017 16:18:54  $
--       Date fetched Out : $Modtime:   08 Mar 2017 16:16:54  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='awlrs1002_awlrs1003_1_&log_extension'
define logfile2='awlrs1002_awlrs1003_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'AWLRS'
                          ,p_new_version           => '1.0.0.3'
                          ,p_allowed_old_version_1 => '1.0.0.2'
                          );
END;
/
...
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1002_awlrs1003_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1002_awlrs1003_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('AWLRS','awlrs1002_awlrs1003_upg.sql','Upgrade from 1.0.0.2 to 1.0.0.3','1.0.0.3');
END;
/
COMMIT;
SET HEADING OFF
