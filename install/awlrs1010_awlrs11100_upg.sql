...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //new_vm_latest/archives/awlrs/install/awlrs1010_awlrs11100_upg.sql-arc   1.0   May 22 2020 10:25:08   Barbara.Odriscoll  $
--       Date into PVCS   : $Date:   May 22 2020 10:25:08  $
--       Module Name      : $Workfile:   awlrs1010_awlrs11100_upg.sql  $
--       Date fetched Out : $Modtime:   May 22 2020 10:19:00  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='awlrs1010_awlrs11100_1_&log_extension'
define logfile2='awlrs1010_awlrs11100_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'AWLRS'
                          ,p_new_version           => '1.1.10.1'
                          ,p_allowed_old_version_1 => '1.0.1.0'
                          ,p_allowed_old_version_2 => '1.1.7.2'
                          ,p_allowed_old_version_3 => '1.1.8.4'
                          ,p_allowed_old_version_4 => '1.1.9.1'
                          );
END;
/
...
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs11100_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs11100_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('AWLRS','awlrs1010_awlrs11100_upg.sql','Upgrade from 1.0.1.0 to 1.1.10.0','1.1.10.1');
END;
/
COMMIT;
SET HEADING OFF
