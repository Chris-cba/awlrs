...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //new_vm_latest/archives/awlrs/install/awlrs1010_awlrs13150_upg.sql-arc   1.0   Jul 23 2020 15:52:42   Mike.Huitson  $
--       Module Name      : $Workfile:   awlrs1010_awlrs13150_upg.sql  $
--       Date into PVCS   : $Date:   Jul 23 2020 15:52:42  $
--       Date fetched Out : $Modtime:   Jul 23 2020 15:50:14  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='awlrs1010_awlrs13150_1_&log_extension'
define logfile2='awlrs1010_awlrs13150_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'AWLRS'
                          ,p_new_version           => '1.3.15.0'
                          ,p_allowed_old_version_1 => '1.0.1.0'
                          ,p_allowed_old_version_2 => '1.3.13.1'
                          );
END;
/
...
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs13150_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs13150_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('AWLRS','awlrs1010_awlrs13150_upg.sql','Upgrade from 1.0.1.0 to 1.3.15.0','1.3.15.0');
END;
/
COMMIT;
SET HEADING OFF
