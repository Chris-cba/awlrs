...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //new_vm_latest/archives/awlrs/install/awlrs1010_awlrs12130_upg.sql-arc   1.2   May 06 2020 16:51:28   Peter.Bibby  $
--       Module Name      : $Workfile:   awlrs1010_awlrs12130_upg.sql  $
--       Date into PVCS   : $Date:   May 06 2020 16:51:28  $
--       Date fetched Out : $Modtime:   May 06 2020 16:51:02  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='awlrs1010_awlrs12130_1_&log_extension'
define logfile2='awlrs1010_awlrs12130_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'AWLRS'
                          ...
                          ,p_allowed_old_version_2 => '1.2.12.1'
                          );
END;
/
...
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs12130_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs12130_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
