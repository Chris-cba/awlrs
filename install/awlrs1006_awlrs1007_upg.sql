...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //new_vm_latest/archives/awlrs/install/awlrs1006_awlrs1007_upg.sql-arc   1.0   Aug 24 2017 11:40:12   Peter.Bibby  $
--       Module Name      : $Workfile:   awlrs1006_awlrs1007_upg.sql  $
--       Date into PVCS   : $Date:   Aug 24 2017 11:40:12  $
--       Date fetched Out : $Modtime:   Aug 24 2017 11:37:56  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='awlrs1006_awlrs1007_1_&log_extension'
define logfile2='awlrs1006_awlrs1007_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'AWLRS'
                          ,p_new_version           => '1.0.0.7'
                          ,p_allowed_old_version_1 => '1.0.0.6'
                          );
END;
/
...
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1006_awlrs1007_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1006_awlrs1007_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('AWLRS','awlrs1006_awlrs1007_upg.sql','Upgrade from 1.0.0.6 to 1.0.0.7','1.0.0.7');
END;
/
COMMIT;
SET HEADING OFF
