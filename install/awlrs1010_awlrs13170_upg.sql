...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //new_vm_latest/archives/awlrs/install/awlrs1010_awlrs13170_upg.sql-arc   1.0   Apr 28 2021 13:33:12   Barbara.Odriscoll  $
--       Module Name      : $Workfile:   awlrs1010_awlrs13170_upg.sql  $
--       Date into PVCS   : $Date:   Apr 28 2021 13:33:12  $
--       Date fetched Out : $Modtime:   Apr 28 2021 13:26:22  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='awlrs1010_awlrs13170_1_&log_extension'
define logfile2='awlrs1010_awlrs13170_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'AWLRS'
                          ,p_new_version           => '1.3.17.0'
                          ,p_allowed_old_version_1 => '1.0.1.0'
                          ,p_allowed_old_version_2 => '1.2.17.1'
                          ,p_allowed_old_version_3 => '1.3.13.1'
                          );
END;
/
WHENEVER SQLERROR EXIT
...
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs13170_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1010_awlrs13170_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('AWLRS','awlrs1010_awlrs13170_upg.sql','Upgrade from 1.0.1.0 to 1.3.17.0','1.3.17.0');
END;
/
COMMIT;
SET HEADING OFF
