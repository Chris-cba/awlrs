...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //new_vm_latest/archives/awlrs/install/awlrs1005_awlrs1006_upg.sql-arc   1.0   06 Jul 2017 21:26:42   Mike.Huitson  $
--       Module Name      : $Workfile:   awlrs1005_awlrs1006_upg.sql  $
--       Date into PVCS   : $Date:   06 Jul 2017 21:26:42  $
--       Date fetched Out : $Modtime:   06 Jul 2017 21:21:50  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='awlrs1005_awlrs1006_1_&log_extension'
define logfile2='awlrs1005_awlrs1006_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'AWLRS'
                          ,p_new_version           => '1.0.0.6'
                          ,p_allowed_old_version_1 => '1.0.0.5'
                          );
END;
/
...
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1005_awlrs1006_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1005_awlrs1006_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('AWLRS','awlrs1005_awlrs1006_upg.sql','Upgrade from 1.0.0.5 to 1.0.0.6','1.0.0.6');
END;
/
COMMIT;
SET HEADING OFF
