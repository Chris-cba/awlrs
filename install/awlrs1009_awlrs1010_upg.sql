...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='awlrs1009_awlrs1010_1_&log_extension'
define logfile2='awlrs1009_awlrs1010_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'AWLRS'
                          ,p_new_version           => '1.0.1.0'
                          ,p_allowed_old_version_1 => '1.0.0.9'
                          );
END;
/
...
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1009_awlrs1010_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1009_awlrs1010_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('AWLRS','awlrs1009_awlrs1010_upg.sql','Upgrade from 1.0.0.9 to 1.0.1.0','1.0.1.0');
END;
/
COMMIT;
SET HEADING OFF
