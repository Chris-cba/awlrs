...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //new_vm_latest/archives/awlrs/install/awlrs1007_awlrs1008_upg.sql-arc   1.0   Oct 06 2017 09:30:20   Peter.Bibby  $
--       Module Name      : $Workfile:   awlrs1007_awlrs1008_upg.sql  $
--       Date into PVCS   : $Date:   Oct 06 2017 09:30:20  $
--       Date fetched Out : $Modtime:   Oct 06 2017 09:28:44  $
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='awlrs1007_awlrs1008_1_&log_extension'
define logfile2='awlrs1007_awlrs1008_2_&log_extension'
spool &logfile1
--get some db info
...
begin
   hig2.pre_upgrade_check (p_product               => 'AWLRS'
                          ,p_new_version           => '1.0.0.8'
                          ,p_allowed_old_version_1 => '1.0.0.7'
                          );
END;
/
...
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1007_awlrs1008_ddl_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET DEFINE ON
SELECT '&exor_base'||'awlrs'||'&terminator'||'install'||
        '&terminator'||'awlrs1007_awlrs1008_metadata_upg.sql' run_file
FROM dual
/
SET FEEDBACK ON
start &&run_file
...
SET TERM OFF
BEGIN
      hig2.upgrade('AWLRS','awlrs1007_awlrs1008_upg.sql','Upgrade from 1.0.0.7 to 1.0.0.8','1.0.0.8');
END;
/
COMMIT;
SET HEADING OFF
